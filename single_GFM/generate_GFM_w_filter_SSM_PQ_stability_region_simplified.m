...
Nmp = 101; % number of points along mp axis
Nnq = 101; % number of points along nq axis
...
% ---------------- Simplified Plot: Feasibility and Plugin-Safe Regions Only ----------------

% Prepare coordinate grids in percent
mp_plot = 100 * mp_vec / Dp(1);  % mp as percent of Dp(1)
nq_plot = 100 * nq_vec / Dq(1);  % nq as percent of Dq(1)
[MP_grid, NQ_grid] = meshgrid(mp_plot, nq_plot);

figure('Name','Feasibility and Plugin-Safe Regions','Position',[100 100 900 600]);
hold on;

% Overlay: Combined Plugin-safe regions (any filter works)
...
if any(combinedPluginMap(:))
    ...
    if ~isempty(h_fill.FacePrims)
        set(h_fill.FacePrims, 'FaceAlpha', 0.4);
        h_fill.FaceColor = pluginColor;
    end
    contour(MP_grid, NQ_grid, combinedPluginMap, [0.5 0.5], 'LineWidth', 2, 'Color', pluginColor);
end

% Overlay: Feasibility region (transparent green patches)
feasMap = double(stabilityFlag' & feasibilityFlag');
if any(feasMap(:))
    feasColor = [0.2 0.8 0.3]; % Green
    [~, h_feas] = contourf(MP_grid, NQ_grid, feasMap, [0.5 1], 'LineStyle', 'none');
    if ~isempty(h_feas.FacePrims)
        set(h_feas.FacePrims, 'FaceAlpha', 0.5);
        h_feas.FaceColor = feasColor;
    end
    contour(MP_grid, NQ_grid, feasMap, [0.5 0.5], 'LineWidth', 1.5, 'Color', [0.0 0.6 0.2]);
end

xlabel('m_p (% of D_p)', 'FontSize', 12);
ylabel('n_q (% of D_q)', 'FontSize', 12);
title('Feasibility & Plugin-Safe Regions', 'FontSize', 14);
grid on;
axis tight;

% Custom legend
legend_entries = {'Feasible', 'Plugin-safe (any filter)'};
legend_handles = [
    patch(NaN, NaN, [0.2 0.8 0.3], 'FaceAlpha', 0.4), ...
    patch(NaN, NaN, [0.0 0.4 0.8], 'FaceAlpha', 0.4)
];
...
resultsStab.filterSet = filterSet;
resultsStab.nu_values = nu_values;
resultsStab.pluginSafeFlag = pluginSafeFlag;
resultsStab.margin = margin;
resultsStab.dominantEig = dominantEig;
resultsStab.tolerance = tol;
resultsStab.J_matrix = J;
resultsStab.w_passivity = w_passivity;
resultsStab.elapsed_seconds = elapsed_scan;

save('stability_region_mp_nq.mat','resultsStab');
fprintf('Stability and passivity region data saved to stability_region_mp_nq.mat\n');
